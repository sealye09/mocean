            ...
            await this.prisma.$transaction(async (tx) => {
                for (const agent of agents) {
                    try {
                        ...
                        errorCount++;
                        console.error(`处理agent失败 (ID: ${agent.id}):`, (error as Error).message);
    ...
    async validateData(): Promise<void> {
        try {
            ...
            recentAgents.forEach(agent => {
                const groups = agent.groupJson ? JSON.parse(agent.groupJson as string) as string[] : [];
                console.log(`  - ${agent.emoji || '🤖'} ${agent.name} (ID: ${agent.id})`);
